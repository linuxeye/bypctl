...
func NewAcmeClient(acmeAccount *models.WebsiteAcmeAccount) (*AcmeClient, error) {
...
func (c *AcmeClient) GetDNSResolve(domains []string) (map[string]Resolve, error) {
	...
	for _, auth := range responses {
		...
		cha, err := challenge.FindChallenge(challenge.DNS01, auth)
		if err != nil {
			resolves[domain] = Resolve{Err: err.Error()}
			continue
		}
		keyAuth, err := core.GetKeyAuthorization(cha.Token)
		if err != nil {
			resolves[domain] = Resolve{Err: err.Error()}
			continue
		}
